...
# -----------------------------
# Comment stripping (tokenize-based, safe for URLs / quotes / etc.)
# -----------------------------
def strip_python_comments(code: str) -> str:
    """
    Remove only true Python comment tokens using tokenize.
    Safe for URLs, '#' inside strings, nested quotes, triple quotes, etc.

    If tokenize fails (snippet may be incomplete), we fall back to removing
    full-line comments only (conservative), and keep inline content unchanged.
    """
    if not code:
        return ""
    ...
    try:
        ...
        # untokenize returns str in py3
        return out
    except (tokenize.TokenError, IndentationError, SyntaxError):
        # Conservative fallback: remove only lines that are entirely comments.
        # This avoids corrupting code when '#' appears inside strings.
...
# -----------------------------
# LOC (non-empty lines) after comment removal
# -----------------------------
...
def nonempty_loc_no_comments(code: str) -> int:
    return nonempty_loc(strip_python_comments(code))
...
_SKIP_TOKEN_TYPES_FOR_COUNT = {
    ...
    tokenize.DEDENT,
    tokenize.ENDMARKER,
    tokenize.COMMENT,  # comment already excluded, but keep here too
}

def python_token_count(code: str) -> int:
    """
    Count Python lexical tokens using tokenize (standard tokens).
    Assumes comments are already removed, but still robust if not.
    Falls back to a whitespace split if tokenize fails.
    """
    if not code:
        return 0
    src = code if code.endswith("\n") else (code + "\n")
...
def has_assertions_tokenwise(code: str) -> bool:
    """
    True if the token NAME 'assert' appears (not inside strings/comments).
    """
    if not code:
        return False
    src = code if code.endswith("\n") else (code + "\n")
    try:
        ...
        # fallback: simple substring (may have false positives in strings)
        return "assert " in code

def has_test_framework_imports_tokenwise(code: str) -> bool:
    """
    Detect pytest/unittest imports using tokens to avoid strings/comments.
    Matches:
      import pytest
      import pytest as p
      from pytest import ...
    ...
    # simple token scan for patterns
    # tokens have .type and .string
    n = len(toks)
    for i in range(n):
        t = toks[i]
        ...
        # "import X"
        if t.string == "import":
            # next NAME tokens might include multiple modules separated by commas
            # We'll check the immediate next NAME that follows.
            j = i + 1
            ...
            if j < n and toks[j].type == tokenize.NAME:
                if toks[j].string in ("pytest", "unittest"):
                    return True
...
def is_test_source_with_reason(src: Dict[str, Any]) -> Tuple[bool, List[str], str, Optional[str], int, int]:
    ...
    code_clean = strip_python_comments(code_raw)

    reasons = test_reasons(file_path, code_clean)
    func_name = extract_first_def_name(code_clean)

    loc = nonempty_loc(code_clean)          # LOC after comment removal
    tok = python_token_count(code_clean)    # token count after comment removal
...
def process_stream(fin, fout, log_exclusions: bool = True):
    ...
    # per-function distributions (after comment removal)
    loc_all: List[int] = []
    loc_kept: List[int] = []
    ...
    for line in fin:
        ...
        # Collect LOC/TOK for ALL sources in this group (after comment removal)
        group_locs: List[int] = []
        group_toks: List[int] = []
        for src in sources:
            code_raw = src.get("code", "") or src.get("func", "")
...
def main():
    parser = argparse.ArgumentParser(
        description="Filter out clone groups that contain Python test functions; report LOC and token distributions (comments removed safely)."
    )
    ...
    # ---- LOC distributions ----
    print_dist_report("LOC: ALL functions (input, comments removed)", loc_all, args.loc_bucket_size, args.loc_topk)
    print_dist_report("LOC: REMAINING functions (output, comments removed)", loc_kept, args.loc_bucket_size, args.loc_topk)
    print_dist_report("LOC: EXCLUDED functions (dropped groups, comments removed)", loc_excluded, args.loc_bucket_size, args.loc_topk)

    # ---- Token distributions ----
    print_dist_report("TOKENS: ALL functions (input, comments removed)", tok_all, args.tok_bucket_size, args.tok_topk)
    print_dist_report("TOKENS: REMAINING functions (output, comments removed)", tok_kept, args.tok_bucket_size, args.tok_topk)
    print_dist_report("TOKENS: EXCLUDED functions (dropped groups, comments removed)", tok_excluded, args.tok_bucket_size, args.tok_topk)
