...
DATASET_ROOT="../dataset/nicad_block_python"
OUT_ROOT="./saved_models_bcb"
#OUT_ROOT="./saved_models_combined_py"

EPOCHS="${1:-2}"
SEED="${2:-3}"

mkdir -p "$OUT_ROOT/predictions_python_block" "$OUT_ROOT/logs"

# ---- one single log file for everything ----
RUN_LOG="$OUT_ROOT/logs/test_all_systems_bcb_python_block.log"
...
for sys in "${systems[@]}"; do
  ...
  preds_rel="predictions_python_block/predictions_${sys}_test.txt"
  ...
  if [[ -f "$preds_abs" ]]; then
    echo "[SKIP] predictions_python_block already exist for $sys: $preds_abs"
