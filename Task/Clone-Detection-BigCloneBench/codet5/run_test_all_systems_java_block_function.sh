...
#OUT_ROOT="./saved_models_bcb/nicad_block_java"
OUT_ROOT="./saved_models_combined/nicad_block_java"

EPOCHS="${1:-2}"
SEED="${2:-3}"

# Global checkpoint to reuse for ALL repos:
CKPT_ROOT="${CKPT_ROOT:-/home/user1-system11/research_dream/llm-clone/Task/Clone-Detection-BigCloneBench/codet5/saved_models_combined/checkpoint-best-f1}"
CKPT_BIN="$CKPT_ROOT/pytorch_model.bin"

mkdir -p "$OUT_ROOT"
RUN_LOG="$OUT_ROOT/test_all_systems_codet5_bcb_java_block_function.log"
...
for sys in "${systems[@]}"; do
  ...
  mkdir -p "$sys_out_dir/cache_data" "$sys_out_dir/predictions_bcb_java_block_function" "$sys_out_dir/tensorboard"
