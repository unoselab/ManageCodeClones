mkdir ./saved_models_combined_py/
python run.py \
    --output_dir=./saved_models_combined_py/ \
    --subsample_ratio 1.0 \
    --model_type=gpt2 \
    --config_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
    --model_name_or_path=microsoft/CodeGPT-small-java-adaptedGPT2 \
    --tokenizer_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
    --do_train \
    --do_test \
    --evaluate_during_training \
    --train_data_file=../dataset/combined_py_plus_org10/train_mix.txt \
    --eval_data_file=../dataset/combined_py_plus_org10/valid_mix.txt \
    --test_data_file=../dataset/org/test.txt \
    --epoch 2 \
    --block_size 400 \
    --train_batch_size 16 \
    --eval_batch_size 32 \
    --learning_rate 5e-5 \
    --max_grad_norm 1.0 \
    --seed 3 2>&1| tee ./saved_models_combined_py/train_combined_py_plus_org10_codegpt.log
   

# Run Train only , remove: #--do_test \ #--test_data_file=../dataset/test.txt \
# mkdir ./saved_models_bcb/
# python run.py \
#     --output_dir=./saved_models_bcb/ \
#     --model_type=gpt2 \
#     --config_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --model_name_or_path=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --tokenizer_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --do_train \
#     --evaluate_during_training \
#     --train_data_file=../dataset/train.txt \
#     --eval_data_file=../dataset/valid.txt \
#     --epoch 2 \
#     --block_size 400 \
#     --train_batch_size 16 \
#     --eval_batch_size 32 \
#     --learning_rate 5e-5 \
#     --max_grad_norm 1.0 \
#     --seed 3 2>&1| tee ./saved_models_bcb/train_bcb.log
   
# mkdir ./saved_models_combined/
# python run.py \
#     --output_dir=./saved_models_combined/ \
#     --subsample_ratio 1.0 \
#     --model_type=gpt2 \
#     --config_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --model_name_or_path=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --tokenizer_name=microsoft/CodeGPT-small-java-adaptedGPT2 \
#     --do_train \
#     --do_test \
#     --evaluate_during_training \
#     --train_data_file=../dataset/combined_org10_plus_repos/train_mix.txt \
#     --eval_data_file=../dataset/combined_org10_plus_repos/valid_mix.txt \
#     --test_data_file=../dataset/org/test.txt \
#     --epoch 2 \
#     --block_size 400 \
#     --train_batch_size 16 \
#     --eval_batch_size 32 \
#     --learning_rate 5e-5 \
#     --max_grad_norm 1.0 \
#     --seed 3 2>&1| tee ./saved_models_combined/train_combined_org10_plus_repos_codegpt.log
   
